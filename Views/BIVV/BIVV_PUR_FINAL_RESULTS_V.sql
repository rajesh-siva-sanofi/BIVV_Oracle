...
SELECT 
   CASE WHEN (SELECT cnt FROM val WHERE val.cont_num = src.cont_num AND val.ndc11 = src.ndc11 AND val.qtr = src.qtr) > 1 THEN 'ERR: Multiple URAs. Adjust query' ELSE 'OK' END AS val_msg
   ,src.*
FROM src
;
...
SELECT
   ...
   ,DECODE(s.cont_num, 1, 1, pm.hcrs_pgm_id) AS pgm_id
