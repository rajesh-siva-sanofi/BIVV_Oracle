CREATE OR REPLACE VIEW BIVV.IC_PRICING_II_V AS
/****************************************************************************
 * View Name    : bivv.ic_pricing_ii_v
 * Date Created : 02.09.2021
 * Author       : JTronoski (IntegriChain)
 * Description  : Bioverativ (FLEX IC) Pricing Master data Version II
 *
 * MOD HISTORY
 *  Date        Modified by   Reason
 *  ----------  ------------  ------------------------------------------------
 *  02.10.2021	JTronoski     Changed the name from ic_pricing_v to ic_pricing_ii_v 
****************************************************************************/
WITH ddr_qtr_data
  AS (-- Extract quarterly amp/bp data from ddr table
      SELECT SUBSTR(period,3,4) || SUBSTR(period,1,1) year_qtr
...
--       ORDER BY 1,2,3
  ...
  ic_mth_data
  AS (-- Merge the product information with the monthly AMP data
      SELECT a.ndc11
            ,a.ndc9
            ,a.year_qtr
            ,b.market_date_ndc9
...
--     ) SELECT COUNT(*) OVER () cnt, ic_mth_data.* 
--         FROM ic_mth_data
--       ORDER BY 1,2,3
     ),
  ic_gp_amp_bp
...
--         FROM ic_amp_bp
  ...
  ic_qtr_data
  AS (-- Merge the AMP Monthly data with the AMP/BP IC Quarterly Data
      SELECT a.ndc11
            ,a.ndc9
            ,a.year_qtr
            ,a.market_date_ndc9
            ,a.first_sale_date_ndc9
            ,a.first_sale_date_ndc11
            ,a.gp_amp_mth_1
            ,a.gp_amp_5i_mth_1
            ,a.amp_mth_1
            ,a.gp_amp_mth_2
            ...
            ,b.amp_qtrly
            ,b.bp_qtrly
            ,add_months(TRUNC(LEAST(a.first_sale_date_ndc9
                                   ,a.first_sale_date_ndc11), 'q'), 3) base_amp_qtr_start
        FROM ic_mth_data a
            ,ic_amp_bp b
       WHERE a.ndc11 = b.ndc11 (+)
         AND a.year_qtr = b.year_qtr (+)
