CREATE OR REPLACE VIEW hcrs.calc_csr_main_pi_v
AS
   SELECT
          /*+ calc_csr_main_pi_v
              QB_NAME( pi )
          ...
          */
   /****************************************************************************
   *    View Name : calc_csr_main_pi_v
   * Date Created : 01/15/2018
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part I (18)
   *
   *                TRANS GROUP
   *
   *                Identify the transaction groups with the root transaction
   *                groups and parent transaction groups.
   *
   ...
   ****************************************************************************/
          ...
          -- Assign Transaction Order within the Parent Group (partitoned by parent)
          -- Trans Class, Source System, Contract, Invoice
          ----------------------------------------------------------------------
          DENSE_RANK()
             OVER (PARTITION BY z.root_trans_grp,
                                z.parent_trans_grp
                       ORDER BY -- Trans identifiers
                                z.trans_cls_cd,
                                z.source_sys_cde,
                                z.lgcy_trans_ord,
                                z.contr_id NULLS FIRST,
                                -- Use absolute value to preserve order due to
                                -- ICW using negatives for RMUS external data
                                ABS( z.lgcy_trans_no),
                                ABS( z.assc_invc_no) NULLS FIRST,
                                z.lgcy_trans_line_no NULLS FIRST) trans_grp
          -- Constants -----------------------------------------------------------------------------
          -- Marking data --------------------------------------------------------------------------
          -- Cursor Row Count ----------------------------------------------------------------------
     FROM hcrs.calc_csr_main_ph_v z
       -- Eliminate rows that cannot qualify for accumulation rules
    WHERE NOT (   (    z.mark_accum_nom_ind = z.flag_yes
                   AND z.chk_nom = z.flag_no
                  )
               OR (    z.mark_accum_hhs_ind = z.flag_yes
                   AND z.chk_hhs = z.flag_no
                  )
               OR (    z.mark_accum_contr_ind = z.flag_yes
                   AND z.chk_contr = z.flag_no
                  )
               OR (    z.mark_accum_fsscontr_ind = z.flag_yes
                   AND z.chk_fss_contr = z.flag_no
                  )
               OR (    z.mark_accum_phscontr_ind = z.flag_yes
