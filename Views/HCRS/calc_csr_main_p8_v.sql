...
AS
   ...
   *                ROOT LOOKUP
   *
   *                Gets the root trans_id for SAP_ADJ, CARS_ADJ, and X360_ADJ
   *                linkages.  These links are done as scalar subqueries because
   *                there may be a heirarchy of transactions between the parent
   *                and the root transaction, there may be more than one root
   *                transaction, the volumne of SAP, SAP4H, and X360 links are
   *                low, and the index used contains all columns required to
   ...
   ****************************************************************************/
          ...
          CASE -- Ordered by frequency of occurance in data
             ...
             THEN z.flag_no
             ---------------------------------------------------------------------------------------
             -- ICW_KEY Linking: The ICW_KEY rebate contract ID must match the related sale/credit contract
             -- ID or the related sale/credit contract ID can be NULL, but they does not need to match
             -- to obtain Gross Dollars/Units/Packages. The customer IDs never need to match.
