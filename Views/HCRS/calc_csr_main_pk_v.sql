CREATE OR REPLACE VIEW hcrs.calc_csr_main_pk_v
AS
   SELECT
          /*+ calc_csr_main_pk_v
              QB_NAME( pk )
          ...
          */
   /****************************************************************************
   *    View Name : calc_csr_main_pk_v
   * Date Created : 01/15/2018
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part K (20)
   *
   *                MINIMUM TRANS DATE 1
   *
   *                Get the marking transaction date for the first transaction in
   *                the group.
   *
   ...
   ****************************************************************************/
          ...
          -- Marking Trans Date --------------------------------------------------------------------
          z.trans_dt,
          CASE -- First trans of a root group
             WHEN z.parent_trans_grp = 1
              AND z.trans_grp = 1
              AND z.trans_ord = 1
             THEN z.trans_dt
          END min_trans_dt,
          ...
          -- Marking data --------------------------------------------------------------------------
          -- Cursor Row Count ----------------------------------------------------------------------
     FROM hcrs.calc_csr_main_pj_v z;
