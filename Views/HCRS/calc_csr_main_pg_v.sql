CREATE OR REPLACE VIEW hcrs.calc_csr_main_pg_v
AS
   SELECT
          /*+ calc_csr_main_pg_v
              QB_NAME( pg )
          ...
          */
   /****************************************************************************
   *    View Name : calc_csr_main_pg_v
   * Date Created : 01/15/2018
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part G (16)
   *
   *                ROOT TRANS GROUP
   *
   *                Identify the root transaction groups within the components
   *
   ...
   ****************************************************************************/
          ...
          -- Trans Groupings -----------------------------------------------------------------------
          ----------------------------------------------------------------------
          -- Identify Root Transaction Groups
          -- Data Source, Customer and COT, Product,
          -- Component and marking and accumulation settings,
          -- Root Trans Class, Root Source System, Root Contract, Root Invoice
          -- Link Transaction date, Link Earn Date
          ----------------------------------------------------------------------
          DENSE_RANK()
             OVER (ORDER BY -- Source
                            ...
                            z.root_trans_cls_cd,
                            z.root_source_sys_cde,
                            z.root_contr_id NULLS FIRST,
                            -- Use absolute value to preserve order due to
                            -- ICW using negatives for RMUS external data
                            ABS( z.root_lgcy_trans_no),
                            -- Link dates
                            z.link_trans_dt,
                            z.earn_bgn_dt) root_trans_grp,
          -- Constants -----------------------------------------------------------------------------
          z.flag_yes,
          z.flag_no
          -- Marking data --------------------------------------------------------------------------
          -- Cursor Row Count ----------------------------------------------------------------------
     FROM hcrs.calc_csr_main_pf_v z;
