CREATE OR REPLACE VIEW HCRS.BIVV_RECON_TOTALS_BY_NDC_QTR_V AS
WITH src AS (
   SELECT p.prod_id_pri AS ndc11
      ...
      ,NVL(SUM(DECODE(i.adjitemint_flg_override, 'Y', i.adjitemint_override_amt, i.adjitemint_calc_amt)),0) AS int_amt
      ,COUNT(*)
...
tgt AS (
   ...
   GROUP BY v.ndc_lbl||v.ndc_prod||v.ndc_pckg, v.period),
cmp AS (
   SELECT
      CASE
      ...
      ,src.ndc11, src.qtr, src.adjitm_auth_units AS flex_auth_units, tgt.paid_units AS hcrs_paid_units
   ...
   FROM src
   LEFT OUTER JOIN tgt ON tgt.ndc11 = src.ndc11 AND tgt.period = src.qtr)
SELECT "VALID_MSG","NDC11","QTR","FLEX_AUTH_UNITS","HCRS_PAID_UNITS","FLEX_PAID_AMT","HCRS_PAID_AMT","FLEX_INT_AMT","HCRS_INT_AMT"
FROM cmp
ORDER BY valid_msg, ndc11, qtr
;
