CREATE OR REPLACE VIEW hcrs.calc_csr_main_p0_0_v
AS
   SELECT
          /*+ calc_csr_main_p0_0_v
              QB_NAME( p0_0 )
              NO_MERGE
              LEADING( ppw z )
              USE_NL( ppw z )
              FULL( ppw )
              INDEX( z mstr_trans_ix501 )
              DYNAMIC_SAMPLING( 0 )
          */
   /****************************************************************************
   *    View Name : calc_csr_main_p0_0_v
   * Date Created : 01/15/2018
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part 0-0
   *
   *                PAID DATE TRANS
   *
   *                Get product transactions with the paid date within the
   *                calculation period date for calculations that use paid date.
   *
   ...
   ****************************************************************************/
          ...
          -- Root Identifiers ----------------------------------------------------------------------
          -- Marking Trans Date --------------------------------------------------------------------
          ppw.flag_yes uses_paid_dt,
          ppw.flag_no uses_earn_dt,
          -- Parent Identifiers --------------------------------------------------------------------
          -- Trans Identifiers ---------------------------------------------------------------------
          z.trans_cls_cd,
          z.source_sys_cde,
    ...
    WHERE ppw.trans_ndc_lbl = z.ndc_lbl
      ...
      AND ppw.snpsht_id >= z.snpsht_id
       -- Limit paid date by paid date range, include extra room for partition pruning
      AND z.paid_dt BETWEEN (ppw.min_paid_start_dt - ppw.prune_days) AND (ppw.max_paid_end_dt + ppw.prune_days)
       --------------------------------------------------------------------
       -- PPW: Don't include NDCs that are for bundling only
      AND ppw.bndl_only = ppw.flag_no
       -- PPW: Only when paid date is used
      AND ppw.uses_paid_dt = ppw.flag_yes;
