CREATE OR REPLACE VIEW hcrs.calc_csr_main_pj_v
AS
   SELECT
          /*+ calc_csr_main_pj_v
              QB_NAME( pj )
          ...
          */
   /****************************************************************************
   *    View Name : calc_csr_main_pj_v
   * Date Created : 01/15/2018
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part J (19)
   *
   *                TRANS ORDER
   *
   *                Order the transactions within the root transaction group,
   *                parent transaction group, and transaction group
   *
   ...
   ****************************************************************************/
          ...
          z.trans_grp,
          ----------------------------------------------------------------------
          -- Assign Order within the Transaction Group (partitoned by trans group)
          -- Original Invoice, Invoice Line, record creation (trans_ids), and
          -- estimate order
          ----------------------------------------------------------------------
          DENSE_RANK()
             OVER (PARTITION BY z.root_trans_grp,
                       ...
                       ORDER BY -- Trans identifiers
                                z.trans_id,
                                z.splt_pct_ord) trans_ord
          -- Constants -----------------------------------------------------------------------------
          -- Marking data --------------------------------------------------------------------------
          -- Cursor Row Count ----------------------------------------------------------------------
     FROM hcrs.calc_csr_main_pi_v z;
