CREATE OR REPLACE VIEW hcrs.calc_csr_main_pl_v
AS
   SELECT
          /*+ calc_csr_main_pl_v
              QB_NAME( pl )
          ...
          */
   /****************************************************************************
   *    View Name : calc_csr_main_pl_v
   * Date Created : 01/15/2018
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part L (21)
   *
   *                MINIMUM TRANS DATE 2
   *
   *                Set the minimum marking transaction date for all rows.
   *
   ...
   ****************************************************************************/
          ...
          -- Marking Trans Date --------------------------------------------------------------------
          MIN( z.min_trans_dt) OVER (PARTITION BY z.root_trans_grp) trans_dt,
          -- Parent Identifiers --------------------------------------------------------------------
          ...
          -- Marking data --------------------------------------------------------------------------
          -- Cursor Row Count ----------------------------------------------------------------------
     FROM hcrs.calc_csr_main_pk_v z;
