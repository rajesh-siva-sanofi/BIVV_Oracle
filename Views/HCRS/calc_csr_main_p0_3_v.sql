CREATE OR REPLACE VIEW hcrs.calc_csr_main_p0_3_v
AS
   SELECT
          /*+ calc_csr_main_p0_3_v
              QB_NAME( p0_3 )
              NO_MERGE
              LEADING( ppw z )
              USE_NL( ppw z )
              INDEX( z mstr_trans_ix02 )
              DYNAMIC_SAMPLING( 0 )
          */
   /****************************************************************************
   *    View Name : calc_csr_main_p0_3_v
   * Date Created : 03/01/2019
   *       Author : Joe Kidd
   *  Description : Main calculation transaction query, part 0-3
   *
   *                EARN DATE TRANS DIR ADJ SALE ICW_KEY
   *
   ...
   ****************************************************************************/
    ...
    WHERE ppw.rec_src_ind = z.rec_src_ind
      ...
      AND ppw.unique_id = z.related_sales_id
      ...
      AND z.earn_bgn_dt BETWEEN (ppw.max_earn_end_dt + ppw.prune_days + 1) AND (ppw.snpsht_dt + ppw.prune_days)
       --------------------------------------------------------------------
       -- PPW: Only Sales rows
      AND ppw.src_tbl_cd = ppw.src_tbl_iis
       -----------------------------------------------------
